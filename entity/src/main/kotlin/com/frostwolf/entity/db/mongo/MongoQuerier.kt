...
class MongoQuerier : Querier {
    ...
    override fun <T : Entity<*>> one(id: Any, clazz: Class<T>): T? = template.findById(id, clazz)

    override fun <T : Entity<*>> all(clazz: Class<T>): List<T>? = template.findAll(clazz)

    override fun <T : Entity<*>> query(clazz: Class<T>, where: String, fields: List<String>): List<T> {
    ...
    override fun <T : Entity<*>, R> query(clazz: Class<T>, projections: Any?, where: Any?, order: Any?, limit: Int?, wrapper: ((Any) -> R?)?): List<R> {
